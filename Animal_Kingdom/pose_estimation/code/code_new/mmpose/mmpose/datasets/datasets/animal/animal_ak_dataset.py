...
class AnimalKingdomDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    ## Configuration and Class modified from AnimalPoseDataset
    """
    ...
    #     """Evaluate keypoint results. The pose prediction results will be
    #     saved in ``${res_folder}/result_keypoints.json``.

    #     '''Code from AnimalFlyDataset'''
    ...
    #             Options: 'PCK', 'AUC', 'EPE', 'mAP'.

    #     Returns:
    #         dict: Evaluation results for evaluation metric.
    #     """
    #     metrics = metric if isinstance(metric, list) else [metric]
    #     allowed_metrics = ['PCK', 'AUC', 'EPE', 'mAP']
    #     for metric in metrics:
    #         if metric not in allowed_metrics:
    #             raise KeyError(f'metric {metric} is not supported')
